...
void LCD_init(uint8_t address) {
    ...
    LCD_send_cmd(0x02 | 0x01);
    LCD_send_cmd(0x02 | 0x01);
    LCD_send_cmd(0x02 | 0x01);
    LCD_send_cmd(0x02);


    LCD_send_cmd(0x20 | 0x08 | 0x00 | 0x00);

    LCD_send_cmd(0x08 | 0x04 | 0x00 | 0x00);

    LCD_send_cmd(0x01);

    LCD_send_cmd(0x04 | 0x18);
...
void LCD_backlight(_Bool on) {
    _LCD_backlight = on ? 0x08 : 0x00;
    I2C_start();
    I2C_select(_LCD_address, 0x00);
...
void LCD_send_cmd(unsigned char command) {
    ...
    I2C_write(nibble_upper | _LCD_backlight | 0b00000100);
    I2C_write(nibble_upper | _LCD_backlight);

    I2C_write(nibble_lower | _LCD_backlight | 0b00000100);
    I2C_write(nibble_lower | _LCD_backlight);
...
void LCD_send_data(unsigned char data) {
    ...
    I2C_write(nibble_upper | _LCD_backlight | 0b00000100 | 0b00000001);
    I2C_write(nibble_upper | _LCD_backlight | 0b00000001);

    I2C_write(nibble_lower | _LCD_backlight | 0b00000100 | 0b00000001);
    I2C_write(nibble_lower | _LCD_backlight | 0b00000001);

    I2C_stop();
}

__attribute__((inline)) void LCD_select_line(uint8_t line) {
    switch(line) {
        case 2:
            LCD_send_cmd(0xC0);
            break;
...
void LCD_send_string(char *str, uint8_t line) {
    while (*str && line > 0 && line <= _LCD_rows) {
        ...
        while (*str && *str != '\n' && col++ < _LCD_cols) {
            LCD_send_data(*str++);
            if (delay == -1) _delay((unsigned long)((500)*(4000000/4000000.0)));
            else for(i = 0; i < delay; i++) _delay((unsigned long)((1)*(4000000/4000.0)));
        }
